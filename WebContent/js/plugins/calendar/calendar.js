...
function CalAddCss(){
		...
		cssHtml += ".calLayout{	border:1px solid #DDDDDD; background-color:#ffffff;	position:absolute;padding:2px;display:none;z-index:9999	}";			
		cssHtml += ".calTitle{ border:0px solid #D4D1BF; background-color: #ffffff; width:99%;	font-size:9pt;}";			
		...
		cssHtml += ".calDay{background-color: #fff;border:1px solid #fff;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;}";
		cssHtml += ".calSunDay{	background-color: #ffffff;border:1px solid #ffffff;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:red;}";
		cssHtml += ".calSatDay{ background-color: #ffffff;border:1px solid #ffffff;	width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;	color:blue;}";
		cssHtml += ".calOtherDay{background-color: #ffffff;	border:1px solid #ffffff;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:#CBC7BD;}";
		cssHtml += ".dayToday{background-color: #6b6b6b; border-radius:5px; border:1px solid #CBC7BD;font-weight:bold;font-weight:bold;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:white;	}";
		cssHtml += ".calSelDay{	background-color: #94bbf9; border-radius:5px; border:0px solid #CBC7BD;font-weight:bold;width:20px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;text-decoration:none;color:white;}";
		...
		cssHtml += ".todayBtn{clear:both;text-align: center;font-size:8pt;cursor:pointer;background-color: #eeeeee;border:1px solid #ffffff;padding:4px 4px 0px 3px;height:15px;}";
...
function initCal(obj){
	...
	var maxYear 	= obj.max == null ? "2030" : obj.max;
	var splitKey 	= obj.splitKey == null ? "-" : obj.splitKey; 
	var todayYN		= (obj.today == null || obj.today == 'n') ? "n" : "y";
	var iconYN		= (obj.icon  == null || obj.icon == 'n') ? "n" : "y";
	//var contextRoot = obj.context  == null ? "" : obj.context;
	//var iconUrl    = obj.iconUrl == null ? "/img/calendar.gif" : obj.iconUrl ; 			// img path modify ellepark
	//var iconUrl    = obj.iconUrl == null ?  contextRoot + "/img/calendar_icon.jpg" : obj.iconUrl ; 		// img path modify ellepark =>
	var iconUrl = "";
	var lang = obj.lang == null ? "KO" : obj.lang;		// added by ellepark
...
// 달력 폼 그리기 added by lang
function drawCalForm(id,tId,calType,min,max,todayYN,iconYN,imageUrl){		
	if(iconYN == "y"){
	...
	// 상단 년,달 이동 
	var html  = "<div class='calTitle'><table width='100%'><tr><td id='"+ id +"before' width='10%'  style='font-size: 10px; text-align:center;'>◀</td><td width='80%' style='text-align:center;'>";
	...
	if( calType == "day"){
	    ...
	    for(var i=1;i<=12;i++){ html += "<option value=" + i + ">" + i + monthNm+"</option>"; }
		html += "</select>"						
	}
	
	html += "</td><td id='"+ id +"next' width='10%' style='font-size: 10px; text-align:center;'>▶</td></tr></table></div>";
