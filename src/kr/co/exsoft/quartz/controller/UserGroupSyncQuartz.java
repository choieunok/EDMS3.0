 ...
 * 
 */
public class UserGroupSyncQuartz extends QuartzJob {
	
	protected static final Log logger = LogFactory.getLog(UserGroupSyncQuartz.class);
	...
	protected void executeJob(JobExecutionContext jobexecutioncontext) {
		...
		String retUser = null;
		String retGroup = null;
		
		try {
			logger.info("UserGroupSyncQuartz START ="+df.format(sTime));
			
			// 1.수행일 기준 감사정책 배치업무 수행여부 체크
			param.put("work_nm",Constant.BATCH_USERGROUP_SYNC);
			...
			if(!isProcess)	{
				...
				quartzService.batchWorkWrite(work_idx,Constant.WORK_BATCH,Constant.BATCH_USERGROUP_SYNC);
				
				// 4. 동기화처리
				// 4-1. 부서동기화
				retGroup = quartzService.syncGroup();
				
				// 4-2. 사용자동기화
				retUser = quartzService.syncUser();
				
				System.out.println("--------------------------------- result ------------------------------");
				System.out.println(retGroup + "   /   " + retUser);
				
				// 5. DB결과값 세팅
				resultMap.put("message", retGroup + "   /   " + retUser);
		...
		logger.info("UserGroupSyncQuartz END ="+df.format(eTime));
